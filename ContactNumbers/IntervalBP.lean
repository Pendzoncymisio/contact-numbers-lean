...
-- `lo` is a parameter name throughout this file. It collides with a mathlib global
-- of the same name that is now deprecated, which makes the deprecation linter fire on
-- our own binders. Renaming is not an option: the generated tree files pass `lo` as a
-- named argument at 1992 call sites.
