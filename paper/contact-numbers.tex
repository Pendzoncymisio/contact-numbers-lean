...
\]
later improved in~\cite{BezdekReid2013}; the leading constant $6$ reflects the kissing
...
\noindent and Theorem~\ref{thm:main} is the conjunction of the four statements
...
theorem groundState_seven : ... -15 ... bipyr7    ...
theorem groundState_eight : ... -18 ... capBipyr8 ...
theorem groundState_nine  : ... -21 ... capBipyr9 ...
\end{Verbatim}

\noindent while Theorem~\ref{thm:conj52} is \texttt{conj52\_six} through
\texttt{conj52\_nine}, of which the last reads
...
theorem conj52_nine :
  ...
  ∧ (∀ X : Finset E3, HardCore X → X.card = 9 → contactCount X = 42 →
      (∀ v ∈ X, 3 ≤ (neighbors X v).card) ∧ 42 ≤ contactCount X)
\end{Verbatim}

\noindent each of which reports the three standard axioms and nothing more. The third
conjunct is Definition~1 of~\cite{BezdekKhan2018} spelled out.
...
pattern in the \emph{shell} of a particle of degree six. Write $c$ for the central
particle and $u_0,\dots,u_5$ for its neighbours; recentring at $c$ and rescaling,
the $u_i - c$ are unit vectors, and the contacts among them are the pairs at inner
product $1/2$.
...
\begin{lemma}[the shell patterns \texttt{P1}, \texttt{P4}, \texttt{P5}]
\label{lem:patterns}
Let $c, u_0, \dots, u_5$ be distinct particles of a hard-core configuration. None of
the following three contact patterns occurs.
\begin{center}
\begin{tabular}{llll}
\toprule
 & contacts among the $u_i$ & $c$ in contact with \\
\midrule
$P_1$ & $03,04,05,12,14,15,23,24,34$ & $u_0,\dots,u_5$ \\
$P_4$ & $01,04,05,12,13,23,25,34,45$ & $u_0,\dots,u_5$ \\
$P_5$ & $03,04,05,12,14,15,23,25,34$ & $u_1,\dots,u_5$ \\
\bottomrule
\end{tabular}
\end{center}
\end{lemma}
...
In each case the recentred shell has rank at most three, so by Lemma~\ref{lem:gram}
every $4\times4$ Gram determinant among the shell vectors vanishes. The prescribed
contacts fix most entries of the Gram matrix; the vanishing determinants then become
polynomial equations in the few free entries. For $P_1$ these force
$x_{02} = x_{13} = -1/3$ and $x_{01} = -7/18$, after which
$\det G[0,1,4,5]$ has roots $1$ and $7/11$, both exceeding the hard-core ceiling
$1/2$. For $P_4$ the six free entries lie on a $6$-cycle; opposite-pair determinants
factor as $(a-1)(b-1)(ab+a+b)$, so hard-core forces $ab+a+b = 0$ exactly, and the
adjacent-pair determinants $12a^2 - 8ab - 4a + 12b^2 - 4b - 5 = 0$ then leave only
inadmissible solutions. $P_5$ is a difference-vector variant with no cone vertex.
Full details are in the formalization, where these three lemmas are
\texttt{pattern\_p1\_impossible},
\texttt{pattern\_p4\_impossible} and
\texttt{pattern\_p5\_impossible}.
\end{proof}

\begin{remark}
Lemmas~\ref{lem:notri}--\ref{lem:patterns} do not appear in the literature in this
form.
\end{remark}
...
applies: the six of \S\ref{sec:seven}; the three shell patterns of
...
degree-order rule above. The resulting tree has $1\,267\,025$ nodes.
...
\paragraph{Twenty-three kill kinds.} The ten of \S\ref{sec:eight} survive. The new ones
...
\url{https://github.com/Pendzoncymisio/contact-numbers-lean}
\TODO{Zenodo DOI once released}. It depends only on Lean~4
...
The development is $20$ modules and $12\,154$ lines. Of these, $5\,777$ lines
($4.8$\,MB) are machine-generated data --- the $20\,349$ certificates of
\S\ref{sec:seven} and the serialized tree of \S\ref{sec:eight} --- and the remaining
$6\,377$ lines are hand-written. There are $271$ kernel evaluation sites
(\texttt{decide +kernel}) and no other computational steps. Building from source, with
\texttt{mathlib} already compiled, takes $29$ minutes of wall-clock time on three cores
of an AMD Ryzen~7 5700G. The serialized tree dominates: its slowest single module takes
$19.5$ minutes on its own.
...
\TODO{table of the ten kinds for $n=8$: payload layout, the Boolean check, and the
lemma of \S\ref{sec:arsenal} that discharges it}
